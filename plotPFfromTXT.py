...
def analysis5_plots(Kvals, mean_K_matrix_c, mean_K_matrix_m,
                    median_K_matrix_c, median_K_matrix_m, mrk, clr, component):
    fig, ax = plt.subplots()
...
def plotPFfromTXT():
    ...
    if componentwise_analysis:
        comps = ['6032-29-7', '137-32-6', '67-56-1', '105-54-4',
                 '78-83-1', '78-92-2', '107-87-9', '71-36-3', '78-93-3',
                 '141-78-6', '64-17-5', '565-80-0', '464-06-2', '123-86-4',
                 '79-20-9', '107-39-1', '534-22-5', '120-92-3', '100-66-3',
                 'BOB-1', 'BOB-2', 'BOB-3']
    else:
        comps = ['all']
    ...
    scenario2 = True
    scenario3 = False
    scenario4 = True  # for mean/median: subdivide x-axis into small intervals, then for each interval compute mean/median value and plot that
    scenario5 = True
    ...
    for kk in range(len(comps)):
        ...
        for jj in range(len(Kvals)):
            ...
            if scenario4 or scenario5:
                ...
                mean_cost, mean_merit, median_cost, median_merit = \
                    analysis4(cost_matrix, merit_matrix, cost, merit, KK, comps[kk])
            ...
            if scenario3:
                    # SCENARIO 3: collect maximum possible merit
                    # for each K componentwise
                    max_merit_poss[jj] = np.max(np.ravel(merit))
                    print(max_merit_poss)

            if scenario2:
                    # SCENARIO 2: find all pareto points for which merit is
                    # within merit_bracket percent of max merit
                    print(merit_bracket)
                    bpldata, boxlabel = analysis2(merit, merit_bracket, bpldata, KK, boxlabel, alldata)

            if scenario1:
                    # SCENARIO 1: find all merits I can get
                    # for cost in [C_statistic - C_eps, C_statistic + C_eps]
        ...
        if scenario5:  # dot eh plots
            analysis5_plots(Kvals, mean_K_matrix_c, mean_K_matrix_m,
                            median_K_matrix_c, median_K_matrix_m, mrk,
                            clr, comps[kk])
