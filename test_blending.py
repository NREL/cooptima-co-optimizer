...
from blend_functions import blend_linear_vec, blend_fancy_vec, \
                            volume_to_other
...
if __name__ == '__main__':
    ...
    if False:
        ...
        for BOB in ['BOB-1', 'BOB-2', 'BOB-3']:
            e10lvl[BOB] = []
            for cas in cas_list:
                ME10, vfrac_E10_equiv = find_e10_blend(cas,
                                                       BOB=BOB,
                                                       spids=spids,
                                                       propvec=propvec)
                print("Merit function, E10, {} = {}; equivalent VF = {} ".
                      format(BOB, ME10, vfrac_E10_equiv))
                if(vfrac_E10_equiv is not None and vfrac_E10_equiv > 0.0 and
                   vfrac_E10_equiv < 1.0):
    ...
    if True:
        ...
        for c, nm in zip(cas_list, nm_list):
            if len(c) > 0:
                ...
                # Sample mf
                samples = []
                ksamp = np.random.normal(-1.25, 0.5, 10000)
                for k in ksamp:
                    jbm = revised_mf(RON=ron, S=sen, HoV=HoV, AFR=AFR,
                                     LFV150=LFV150, PMI=PMI, K=k)
                ...
                comp_25.append(np.percentile(samples, 25))
                comp_50.append(np.percentile(samples, 50))
                comp_75.append(np.percentile(samples, 75))
                comp_name.append(nm)
                comp_idx.append(isamp)
                comp_samples.append(samples)

                jbm = revised_mf(RON=ron, S=sen, HoV=HoV,
                                 AFR=AFR, PMI=PMI, K=-1.25)
                mf_knom.append(jbm)
        ...
        plt.close()
        plt.plot(comp_idx, comp_50, 'b.')
        plt.plot(comp_idx, comp_25, 'k+')
        plt.plot(comp_idx, comp_75, 'k+')
        plt.savefig('mf_quartiles.pdf')
        plt.close()

        plt.hist(ksamp, bins=60, alpha=0.4, edgecolor='None', normed='True')
        plt.xlabel('K')
        plt.ylabel("Normalized frequency")
        plt.title("Distribution of K")
        plt.savefig('ksamp.png')

        plt.figure(figsize=(12.0, 16.0))
        for nm, s in zip(comp_name, comp_samples):
            plt.hist(s, bins=60, alpha=0.4, edgecolor='None',
                     normed='True', label=nm)
        plt.xlabel('Merit function score')
        plt.ylabel("Normalized frequency")
        plt.title(' Merit Function Scores for 10%/vol Blend')
        plt.ylim([0, 4])
    ...
    if False:
        ...
        my_names = [u'Ethyl butanoate', u'Diisobutylene', u'2-butanone (MEK)',
                    u'Ethanol', u'2-pentanol', u'Methanol', u'Butyl acetate',
                    u'2-butanol', u'2.4 dimethyl-3- pentanone',
                    u'2-Me-1-butanol', u'sBOB', u'CARBOB', u'wCBOB',
                    u'Isobutanol', u'2-pentanone', u'1-butanol',
                    u'Ethyl acetate', u'Triptane', u'Methyl acetate',
                    u'Anisole', u'Methyl furan', u'Cyclopentanone']
        nsga_names = [u'2.4 dimethyl-3- pentanone', u'2-Me-1-butanol',
                      u'sBOB', u'CARBOB', u'Methanol', u'Ethyl butanoate',
                      u'Isobutanol', u'2-butanol', u'2-pentanone',
                      u'1-butanol', u'2-butanone (MEK)', u'Ethyl acetate',
                      u'Ethanol', u'2-pentanol', u'Triptane', u'wCBOB',
                      u'Butyl acetate', u'Methyl acetate', u'Diisobutylene',
                      u'Anisole', u'Methyl furan', u'Cyclopentanone']

        x = np.zeros_like(x_shuff)
        for nm, xs in zip(nsga_names, x_shuff):
            spot = my_names.index(nm)
            print("{} goes in spot {}, which has name {}".
                  format(nm, spot, my_names[spot]))
            x[spot] = xs
