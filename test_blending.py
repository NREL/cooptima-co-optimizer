...
#        plt.show()
    ...
    if True:
        ...
        # 
                ...
                for k in ksamp:
                    jbm = revised_mf(RON=ron, S=sen, HoV=HoV, AFR=AFR, LFV150=LFV150,
                        PMI=PMI, K=k)
                #print ("jim_bob_merit = {}".format(jbm))
                    mf_vals.append(jbm)
                    mf_idx.append(isamp)
                ...
                jbm = revised_mf(RON=ron, S=sen, HoV=HoV, AFR=AFR, PMI=PMI, K=-1.25)
                mf_knom.append(jbm)
        ...
        for nm, s in zip(comp_name,comp_samples):
            plt.hist(s,bins=60,alpha=0.4,edgecolor='None',normed='True',label=nm)
