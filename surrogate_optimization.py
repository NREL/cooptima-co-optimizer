...
def predict_rbf(CandPoint, data):
    ...
    if data.polynomial == 'none':
        ...
        PolyPart = np.concatenate((np.ones((numpoints, 1)),
                                   CandPoint), axis=1) * data.ctail
    else:
...
def sample_selection(data, CandPoint):
    ...
    if MinCandValue == MaxCandValue:
        ...
        ScaledCandValue = (CandValue - MinCandValue) / \
                          (MaxCandValue - MinCandValue)
    ...
    if data.n_samp == 1:
        ...
        # assign bad scores to candidate points that are too
...
def surrogate_optimization(KK, propvec,  pDB):
    ...
    # for equality constrained case, need dimension d-1
    # because of sum=1 constraint
    data.dim = len(propvec['COST'])-1

    # caution when there are other objectives that have different
    ...
    # may integrate this in generation of candidate points
    # data.constraint = constraint_function
    ...
    # probability to perturb each variable of xbest
    data.pertP = 1
    ...
    data.rbf_weight = 1.0
    # recast the problem as one with N-1 parameters and
    # optimize in (N-1)-dimensional space
    ...
    while rank_s < data.dim+1:
        ...
        startpoints = lhs(data.dim+1, samples=n0, criterion="maximin")
        # sum=1 constraint
        p_init = np.divide(startpoints,
                           np.asmatrix(np.sum(startpoints, axis=1)).T)
        rank_s = np.linalg.matrix_rank(np.concatenate((p_init[:, 0:data.dim],
                                                       np.ones((n0, 1))),
                                                      axis=1))
    ...
    f0 = np.zeros((n0, 1))
    for ii in range(n0):
        f0[ii, 0] = data.objfunction(np.ravel(p_init[ii, 0:data.dim]),
                                     propvec, KK)
    ...
    data.S = np.concatenate((data.s,
                            ...
                            axis=0)
    data.Y = np.concatenate((data.y,
                            np.zeros((data.maxeval - data.m,        1))),
                            axis=0)
    ...
    while data.m < data.maxeval and localminflag == 0:
        ...
        # replace large function values by the median of all
        # available function values--skip for now
        ...
        a_part1 = np.concatenate((PHI[0:data.m, 0:data.m],
                                  P[0:data.m, :]), axis=1)
