    ...
    #    print "Parsing pure component"
        ...
        if input_name is 'MOLWT':
            try:
                ...
                print "Could not convert:", comp['Pure_Molecular_Weight']
                return None
        ...
        if input_name is 'HoV':
            try:
                return float(comp['Pure_Heat_of_Vaporization'])/mf_pc_inputs('MOLWT',comp)*1000
            except ValueError:
                return None
        ...
        if input_name is 'S':
            try:
                return float(re.sub("[^0-9^.]","",comp['Pure_RON'])) - float(re.sub("[^0-9^.]","",comp['Pure_MON']))
            except ValueError:
                return None
        ...
        # bRON(vol_fract) that takes the bRON_pts and finds bRON for that level
        # then blend is sum(b_RON*vol_fracts)

        # to debug need to be able to generate bRON(volfract)
        # and also calc RON(volfract, BOB)

        # Going to also need a function to convert molefract to volfract to energyfract to massfract
        # in context of blending model
        # Going to need to know about the rest of the mixture...

        # Need something like this that can read in blends - can I find the blends
        # in the database?
...
def write_propDB_remap(output_file,fpdb_extract):
    ...
    i = 0
    keys = ['CAS','pk_UUID','co_ID','NAME','BOB','FORMULA','MOLWT','BP','LHV','RON','S',
            'HoV','SL','LFV150','PMI','C','H','O','AFR_STOICH',
            'DENSITY']
...
if __name__ == '__main__':
    ...
    keys = ['Pure_CAS','pk_UUID','Pure_Molecular_Formula','Pure_IUPAC_name',
     ...
     'Blend_Name','Blend_Density','Blend_HoV','Blend_LHV','Blend_MON',
     'Blend_RON','Blend_PMI']
    ...
    # Now re-read that file, combine with the bRON data and write out the result:
