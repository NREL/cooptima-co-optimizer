...
def wrapper_func(n, varID, propDB, KK):
    np.random.seed(n)
    # only randomly sample cost for one (change_name) fuel component
    ncomp, spc_names, propvec = \
        make_property_vector_all_sample_cost_UP_single(propDB, change_name)
    # print(propvec['COST'], ncomp)

    Pfront = run_optmize_nsga2(KK, propvec, propDB)
    return Pfront, propvec, spc_names


def wrapper_func_all(n, propDB, KK):
    # randomly samples cost for all fuel components
    np.random.seed(n)
...
if __name__ == '__main__':
    ...
    if cooptimizer_input.task_list['cost_vs_merit_Pareto']:
        ...
        for KK, col, mk in zip(cooptimizer_input.KVEC,
                               clr[0:nK+1], mrk[0:nK+1]):
            ...
            # make scatter plot if we have only 2 objectives
            if front.shape[1] == 2:
                fig = plt.figure(figsize=(10, 8))
                # plt.scatter(front[:,0], front[:,1],
                #             label="K={}".format(KK), marker=mk, color=col)
        ...
        if front.shape[1] == 2:
            ...
            plt.savefig(cooptimizer_input.cost_vs_merit_plotfilename,
                        form='pdf')
    ...
    if cooptimizer_input.task_list['mean_vs_var_Pareto']:
        ...
        if front.shape[1] == 2:
            ...
            plt.savefig(cooptimizer_input.mean_vs_var_merit_plotfilename,
                        form='pdf')
            output_files.append(
                         cooptimizer_input.mean_vs_var_merit_plotfilename)

    # UQ for individual cost components (assuming uncertainty in
    # only a single component while all others are deterministic)
        ...
        for KK, col, mk in zip(cooptimizer_input.KVEC,
            ...
            if cooptimizer_input.use_pyomo:
                ...
                for varID in range(ncomp):
                    ...
                    pareto_txt = 'merit_cost_pareto_data_' + change_name + \
                                 '_K_' + str(KK) + '.txt'
                    output_files.append(pareto_txt)
                    spdfile = open(pareto_txt, 'w')
    ...
    if cooptimizer_input.task_list['cost_vs_merit_Pareto_UP']:
        ...
        for KK, col, mk in zip(cooptimizer_input.KVEC,
            ...
            if cooptimizer_input.use_pyomo:
                ...
                Front = []
                pool = Pool()
                pool_res = pool.map_async(wrapper_func_all,
                                          ((ns, propDB, KK) for ns in range(cooptimizer_input.nsamples)))
                result = pool_res.get()
                pool.close()
                for ns in range(cooptimizer_input.nsamples):
                    Front.append(result[ns][0])
                    ...
                    pareto_txt = 'merit_cost_pareto_data_all_K' + str(KK) + \
                                 '_sample_' + str(ns) + '.txt'
                    spdfile = open(pareto_txt, 'w')
            ...
            if madeplot:
                ...
                plt.title("Co-Optimizer GA Pareto Front")
                pltname = \
                    cooptimizer_input.cost_vs_merit_Pareto_UP_plotfilename + \
                    '_K_' + str(KK) + '.pdf'
                output_files.append(pltname)
                plt.savefig(pltname, form='pdf')
                plt.close()
        ...
        # this needs to be updated (only active when pyomo is used)
        if not(printed):
            # plt.legend(loc=8, ncol=3, fontsize=10)
            plt.savefig(cooptimizer_input.cost_vs_merit_Pareto_UP_plotfilename +
                        '_K_'+str(KK)+'.pdf')
            costarray = np.array(costlist)
            plt.close()
            with PdfPages('cost_samples.pdf') as pdf:
    ...
    if cooptimizer_input.task_list['K_vs_merit_sweep']:
        ...
        if cooptimizer_input.use_deap_NSGAII:
            ...
            plt.scatter(cooptimizer_input.KVEC, F, marker='^',
                        c='b', s=40, label='surrogate optimizer')
            plt.xlabel('K')
            plt.ylabel('Merit')
    ...
    if cooptimizer_input.task_list['UPMO']:
        ...
        for KK, col, mk in zip(cooptimizer_input.KVEC,
            ...
            if(cooptimizer_input.use_pyomo and
               cooptimizer_input.use_deap_NSGAII):
                print("Choose only 1 optimizer method")
                print("(not use_pyomo and use_deap_NSGAII)!")
                sys.exit(-2)
            ...
            if cooptimizer_input.use_deap_NSGAII:
                ...
                # ,  sen=sen_samples, ref=ref_samples)
                # first output is merit, second is variance
                front = run_optmize_nsga2(KK, propvec, propDB)
                print(front)
            ...
            for stc in range(front.shape[1]-1):
                st1 = st1 + ',{}'
            st1 = st1 + '\n'
            pareto_txt = 'sampling_pareto_uncertfuelprop_mean_var_K_' + \
                str(KK) + '.txt'
            spdfile = open(pareto_txt, 'w')
            for lin in range(front.shape[0]):
                f_write = list()
