...
def clustering(data, CandPoint, Pfront):
    ...
    dist_val = np.transpose(scp.distance.cdist(CandPoint[:, 0:data.dim],
                                               data.S[0:data.m, :]))
    ...
    for ii in range(data.n_samp):
        ...
        if len(group_id) > 0:
            ...
            # print(ingroup)
            # group centroids in parameter space will add up to 1 because of
            # mean values and sum over the matrix
            group_cent[ii, :] = np.mean(ingroup, axis=0)

    # print(group_cent, np.sum(group_cent,axis = 1))
    # check that group centroids are not too close to already evaluated point
    dist_val = np.transpose(scp.distance.cdist(group_cent[:, 0:data.dim],
                                               data.S[0:data.m, :]))
    min_dist_val = np.asmatrix(np.amin(dist_val, axis=0)).T
    del_id = np.where(min_dist_val <= data.tolerance)[0]
    if len(del_id) > 0:
...
def sample_selection(data, CandPoint):
    ...
    if data.n_samp == 1:
        ...
        CandTotalValue = data.rbf_weight * ScaledCandValue +\
            (1 - data.rbf_weight) * ScaledCandMinDist
...
def surrogateMO_optimization(KK, propvec,  pDB):
    ...
    # for equality constrained case, need dimension
    # d-1 because of sum=1 constraint
    data.dim = len(propvec['COST'])-1

    # Caution when there are other objectives that have
    ...
    # number of samples to generate per iteration -- for now 1, serial
    data.n_samp = 4
    ...
    # maximal number of shrikage of standard deviation for normal distribution
    ...
    data.rbf_weight = 1.0
    # recast the problem as one with N-1 parameters
    # and optimize in (N-1)-dimensional space

    # initial design #dimension+1
    startpoints = lhs(data.dim+1, samples=n0, criterion="maximin")
    # sum=1 constraint
    p_init = np.divide(startpoints, np.asmatrix(np.sum(startpoints, axis=1)).T)
    rank_s = np.linalg.matrix_rank(np.concatenate((p_init[:, 0:data.dim],
                                                  np.ones((n0, 1))), axis=1))

    # regenerate initial design until rank condition satisfied
    while rank_s < data.dim+1:
        # initial design #dimension+1
        startpoints = lhs(data.dim+1, samples=n0, criterion="maximin")
        # sum=1 constraint
        p_init = np.divide(startpoints, np.asmatrix(np.sum(startpoints, axis=1)).T)
    ...
    for ii in range(n0):
        ...
        if len(P) == 0:
            ...
            for jj in range(P.shape[0]):
                if (f0f1[ii, :] < P[jj, :]).all():
                    # new point dominates what's in P
                    del_index.append(jj)
                elif (f0f1[ii, :] > P[jj, :]).all():
                    # new point dominates what's in P
                    do_not_include = True
            if len(del_index) > 0:
    ...
    plt.scatter(np.ravel(P[:, 0]), np.ravel(P[:, 1]),
                marker='o', c='r', s=40, label='Pareto set')
    plt.scatter(np.ravel(f0f1[:, 0]), np.ravel(f0f1[:, 1]),
                marker='^', c='b', s=40, label='all points')
    plt.xlabel('Merit')
    plt.ylabel('Cost')
    plt.legend(loc=0, fontsize=10)
    plt.show()
    ...
    # fit surrogate model
    # this is the sample site matrix we fit the RBF to (dimension -1)
    data.s = p_init[:, 0:data.dim]
    data.y = f0f1  # initial vector of function values
    data.m = n0
    data.S = np.concatenate((data.s,
                            np.zeros((data.maxeval - data.m, data.dim))),
                            axis=0)
    data.Y = np.concatenate((data.y, np.zeros((data.maxeval - data.m,
                            data.nobj_chp+data.nobj_exp))),
                            axis=0)
    data.Pfront = P
    data.Ppoints = Px

    # fit surrogate surface
    PairwiseDistance = scp.distance.cdist(data.S[0:data.m, :],
                                          data.S[0:data.m, :], 'euclidean')
    ...
    while data.m < data.maxeval and localminflag == 0:
        ...
        # first columns are expensive, rest is cheap
        for ii in range(data.nobj_exp):
        ...
        a_part1 = np.concatenate((PHI[0:data.m, 0:data.m], P[0:data.m, :]),
                                 axis=1)
        a_part2 = np.concatenate((np.transpose(P[0:data.m, :]),
                                  np.zeros((pdim, pdim))),
                                 axis=1)
        ...
        for ii in range(data.nobj_exp):
            coeff = np.linalg.solve((a + eta * np.eye(data.m + pdim)),
                                    np.concatenate((np.asmatrix(Ftransform[:, ii]).T,
                                                    np.zeros((pdim, 1))), axis=0))
            # print (coeff.shape)
            C[:, ii] = np.ravel(coeff)
        # rbf parameters
        ...
        # returns matrix with Pareto points and Pareto function values
        CandPoint, Pfront = surrogate_mo(data, KK, propvec, pDB)
        ...
        # just for scatterplot
        Fnew = np.zeros((xselected.shape[0], data.nobj_chp+data.nobj_exp))
        for ii in range(xselected.shape[0]):
            ...
            for jj in range(data.Pfront.shape[0]):
                if (Fselected < data.Pfront[jj, :]).all():
                    # new point dominates what's in P
                    del_index.append(jj)
                elif (Fselected > data.Pfront[jj, :]).all():
                    # new point dominates what's in P
                    do_not_include = True
                    break
            if len(del_index) > 0:
        ...
        plt.scatter(np.ravel(-data.Pfront[:, 0]), np.ravel(data.Pfront[:, 1]),
                    marker='o', color='r', s=70, label='Pareto front')
