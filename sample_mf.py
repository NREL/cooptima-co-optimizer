...
def samples_mmf_refsen(bplist, ref_mean, ref_var, sen_mean, sen_var, nsamples,k):
...
if __name__ == '__main__':
    ...
    for bp, M, bpname in zip(bplist, Mlist, bpnames):
        axs[1].hist(M,bins=max(20,len(Mlist[0])/1000),alpha=0.3,edgecolor='None',normed='True',label=bpname)

    for bp, k, bpname in zip(bplist, klist, bpnames):
        axs[0].hist(k,bins=max(20,len(klist[0])/1000),alpha=0.3,edgecolor='None',normed='True',label=bpname)
    ...
    Mlist, slist = samples_mmf_refsen(bplist, ref_mean, ref_var, sen_mean, sen_var, nsamples,-0.5)
    ...
    def mk_subplot(axs,f,i,nr,nc,l):
        ii = i % nr
        jj = (i-ii)/nr
        axs[jj,ii].hist(f,bins=max(20,len(klist[0])/1000),alpha=0.4,edgecolor='None',normed='True')
    ...
    for bp, M, bpname, samples in zip(bplist, Mlist, bpnames, slist):
        ...
        mk_subplot(axs,samples['senPMIFIX'],5,nc,nr,'PMI Fixed Sensitivity')
        mk_subplot(axs,samples['senPMIVAR'],6,nc,nr,'PMI Variable Sensitivity')
    ...
    for bp, M, bpname, samples in zip(bplist, Mlist, bpnames, slist):
        plt.hist(M,bins=max(20,len(klist[0])/1000),alpha=0.4,edgecolor='None',normed='True')
